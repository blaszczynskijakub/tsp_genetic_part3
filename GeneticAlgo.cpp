...
void GeneticAlgo::nextGeneration() {
    ...
    for (int i = 0; i < population.size(); i++) {
        ...
        const std::vector<double> orderA = pickOne(population, fitness);
        const std::vector<double> orderB = pickOne(population, fitness);
        const std::vector<double> order = crossOver(orderA, orderB);

        mutate(const_cast<std::vector<double> &>(order), 0.01);
...
std::vector<double> GeneticAlgo::pickOne(const std::vector<std::vector<double>>& list, const std::vector<double>& prob) {
    int index = 0;
...
std::vector<double> GeneticAlgo::crossOver(const std::vector<double>& orderA, const std::vector<double>& orderB) {
...
//            cout<<input[0][0];
    ...
    for(int i=0;i<cities;i++){
        ...
        std::shuffle(this->population.at(i).begin()+1, this->population.at(i).end(), std::default_random_engine(seed) ) ;
